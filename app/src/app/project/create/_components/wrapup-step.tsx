...
import { FiAlertTriangle, FiCalendar, FiStar } from "react-icons/fi";
import { Button } from "@/components/ui/button";
import { useI18n } from "@/i18n/useI18n";
import NextStepButton from "./next-step-button";

type WrapupStepProps = {
  onNext: () => void;
  onBack?: () => void;
  isNextLoading?: boolean;
};

export default function WrapupStep({ onNext, onBack, isNextLoading }: WrapupStepProps) {
  const { t } = useI18n();
  ...
  return (
    <div className="relative flex flex-col items-center justify-center gap-6 overflow-hidden rounded-3xl border border-[#e1d7c9] bg-[radial-gradient(circle_at_top,#fef6e7,transparent_70%),linear-gradient(135deg,#fff9ef,#f4ecdf)] p-8 text-center text-[#3a2f25] shadow-[0_20px_60px_-40px_rgba(74,52,30,0.45)]">
      ...
      <span className="relative flex h-16 w-16 items-center justify-center rounded-3xl border border-amber-200 bg-amber-50 text-amber-700 shadow-[0_12px_30px_-18px_rgba(125,90,30,0.55)]">
        <FiCalendar className="h-8 w-8" />
      </span>
      <div className="relative space-y-2">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8d7b68]">
          {t("project.create.wrapup.interlude")}
        </p>
        <h3 className="text-3xl font-semibold text-[#2a241f]">
          {t("project.create.wrapup.title")}
        </h3>
      </div>
      <p className="relative max-w-2xl text-sm leading-relaxed text-[#3a2f25]">
        {t("project.create.wrapup.description")}
      </p>
      <div className="relative grid gap-3 text-sm text-[#2a241f] md:grid-cols-3">
        <div className="flex items-center gap-3 rounded-2xl border border-[#eadfd3] bg-white/70 px-4 py-3">
          ...
          {t("project.create.wrapup.items.chances")}
        </div>
        <div className="flex items-center gap-3 rounded-2xl border border-[#eadfd3] bg-white/70 px-4 py-3">
          ...
          {t("project.create.wrapup.items.threats")}
        </div>
        <div className="flex items-center gap-3 rounded-2xl border border-[#eadfd3] bg-white/70 px-4 py-3">
          ...
          {t("project.create.wrapup.items.terms")}
        </div>
      </div>
      <div className="flex w-full flex-wrap items-center justify-between gap-3">
        ...
        <NextStepButton
          label={t("project.create.wrapup.next")}
          onClick={onNext}
